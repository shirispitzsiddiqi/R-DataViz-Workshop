...
```{r, eval=F}
djt %>% 
  filter(!is.na(eth_minor)) %>% 
  ggplot(aes(x=vote2016, y=djt_app, color=eth_minor, group=eth_minor)) +
  stat_summary(geom="point", position=position_dodge(width=.5), fun.data="mean_se", na.rm=T) +
  stat_summary(geom="point", position=position_dodge(width=.5), fun.data="mean_se", na.rm=T) +
  theme_classic() +
  coord_cartesian(ylim=c(1,4)) +
  labs(color="Racial minority status",
       y="Approval of Donald Trump") +
  theme(legend.position="bottom") +
  scale_color_discrete(type=c("palevioletred", "cyan4"))
```
...
I like to save out figures by running the code so it shows up in the Viewer pane, then I use `ggsave`, which saves the last shown figure to a path of your choosing. In an RMarkdown file, that means copy-pasting the code into the console. In a regular R script, you would just run the code and it shows in the Viewer pane.
...
```{r}
# Type "p1" in the console and hit enter. A figure should show up in the Viewer pane.
ggsave("figures/policyratings.png")
```
